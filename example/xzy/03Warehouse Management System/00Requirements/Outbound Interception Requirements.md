...
- 为什么要做这件事：
  - 仓库在复核前、打包前、交接前三个关键节点都可能收到拦截诉求，且是否允许拦截要按仓库配置判断。
  - 这些诉求来自 OMS 的发货订单状态变化，不是 OMS 侧新增拦截单。
  - WMS 必须在货物离仓前阻断后续出库动作，避免已经取消或暂停的订单继续流转。
  - 复核台拦截时，现场需要把同一出库单的实物和拦截标识放在同一个暂存库位，避免一单多品多件被拆散。

- 当前业务或系统问题：
  - 复核前、打包前、交接前这三类状态对拦截对象不一样，不能只用一个统一判断。
  - 不同仓库对可拦截节点的要求不同，需要按仓库维度单独配置是否允许在复核前、打包前、交接前拦截。
...
- 希望达到什么结果：
  - WMS 能判断当前出库单或包裹是否处于仓库配置允许的出库拦截节点。
...
- 现在具体卡在哪里：
  ...
  - 复核完成未打包不作为独立场景。
  - 打包前拦截的是已进入打包流程但尚未提交打包完成的包裹。
  - 已完成打包但尚未交接的包裹归入交接前拦截场景。
  - 交接前拦截的是已进入交接流程但尚未完成物流商揽收确认的包裹。
...
| WMS | 按仓库配置判断可拦截节点；提供复核、打包和交接作业页面；阻断仓内作业；记录内部拦截事件；回传结果 |
...
- 配置项：
  ...
  - 交接前拦截：允许 / 不允许。
- 生效规则：
  - WMS 先读取出库单所属仓库的拦截节点配置，再判断当前作业节点。
  - 仓库未开启的节点不允许拦截成功，必须回传 `NODE_NOT_ALLOWED`。
  - 本文中的“打包前”指打包提交前；已完成打包但尚未交接的场景归入“交接前”。
- 默认值：待确认。
...
|---|---|---|---|
| A1 | OMS 发货订单状态变化，WMS 已存在出库单 | 接收并匹配当前仓库配置和当前出库节点 | 拦截成功或拦截失败 |
...
- 以下 7.2 至 7.4 场景均以对应仓库已开启该节点拦截为前提；未开启时，WMS 必须回传拦截失败，失败原因码为 `NODE_NOT_ALLOWED`。
...
| C2 | 非手工单不存在打包前节点 | 不适用 | 不作为拦截场景 |

说明：

- 打包前节点只会出现在手工单作业流程中，且存在人机交互入口。
- 已完成打包但尚未交接的场景归入交接前拦截场景，不单列为打包前。
...
- 仓库拦截节点配置按仓库维度生效，配置项为复核前、打包前、交接前三个开关。
- 本文中的“打包前”指打包提交前；已完成打包但尚未交接的场景归入交接前场景。
- 本需求不覆盖多包裹订单拦截。
- 已完成交接后，不再允许拦截成功。
...
3. WMS 先读取出库单所属仓库的拦截节点配置，再判断当前作业节点是否允许拦截。
4. 如果仓库未开启对应节点，WMS 回传拦截失败。
5. 如果已完成交接，WMS 回传拦截失败。
6. 如果未完成交接，WMS 阻断复核、打包或交接动作。
7. 如果是复核台一单多品多件订单拦截，WMS 锁定整单，并要求同一出库单全部应拦截货品归集到同一暂存库位。
8. 复核台归集完成后，WMS 打印拦截标识小纸条，并记录模板、语言和打印状态；纸条必须随货投入同一暂存库位。
9. 对于交接场景，WMS 在交接页面上执行上游撤销/拦截，并同步到物流商揽收系统或 TMS；最终揽收确认结果由外部系统回传到 WMS 页面。
10. WMS 向 OMS 回传拦截成功。
11. WMS 留存内部拦截记录，并保留标识单对象供后续扫码追溯。
...
- 字段来源：
  ...
  - WMS 仓库配置：仓库拦截节点配置、配置开关、生效仓库、配置人、配置时间。
...
| 出库拦截相关当前作业节点 | 复核前 / 打包前 / 交接前 / 已交接 | 仅用于出库拦截判断，不代表 WMS 全量作业节点 |
| 仓库拦截节点配置 | 复核前 / 打包前 / 交接前 | 仓库级可拦截节点开关 |
| 配置开关 | 允许 / 不允许 | 控制对应节点是否可拦截 |
| 是否手工单 | 是 / 否 | 区分是否存在打包前节点 |
...
| NODE_NOT_ALLOWED | 当前仓库未开启该节点，或当前节点不允许拦截 |
...
| 系统管理员 | 配置仓库拦截节点开关、标识单模板尺寸和多国语言内容 |
...
| 验收点 | 通过标准 |
|---|---|
| 仓库级节点配置 | WMS 能按仓库维度配置复核前、打包前、交接前是否允许拦截，未开启节点回传 `NODE_NOT_ALLOWED` |
| 复核前拦截 | WMS 能在仓库开启复核前拦截时阻断复核流程并回传拦截成功 |
| 打包前拦截 | WMS 能在仓库开启打包前拦截时阻断手工打包提交并回传拦截成功 |
| 交接前拦截 | WMS 能在仓库开启交接前拦截时在交接页面执行上游撤销/拦截并通过物流商揽收系统或 TMS 阻断交接确认并回传拦截成功 |
| 已交接场景 | WMS 回传拦截失败 |
...
| 场景口径 | 复核完成未打包不作为独立场景，打包前仅限手工单，已完成打包但未交接归入交接前场景 |
